...
| `control/pump/auto` | `ON`, `OFF`, or `DELAY`; published by Node-RED and accepted only in AUTO |
...
- The pump remains OFF if the Node-RED weather decision becomes stale.
...
- Each AUTO watering pulse is limited to 10 seconds, followed by a 60-second soil soak period before re-evaluation.
